...
export default function MovieCard({ movie, isBookmarked, onToggleWatchlist, onClick }) {
  ...
  return (
    ...
    >
      {/* Favori / Watchlist Butonu */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onToggleWatchlist(movie);
        }}
        className={`absolute top-2.5 right-2.5 z-10 p-2 rounded-full backdrop-blur-md transition-all duration-200 cursor-pointer border ${
          isBookmarked
        ...
        title={isBookmarked ? 'Listeden Çıkar' : 'İzleme Listesine Ekle'}
      >
        <Bookmark
